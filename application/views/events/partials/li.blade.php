<li class='container'>
	<div class='item-wrap row'>
		...
		<div class='span4 response'>
			<a class='btn btn-large' rel='tooltip' title='Responses: yes / no / not replied' href='{{ action('invitations@event',array($event->id)) }}'><span class='green'>{{ (int)$event->count_yes }}</span> / <span class='red'>{{ (int)$event->count_no }}</span> / {{ (int)$event->count_noreply }}</a>
			<small><i class='icon-group'></i> Total Confirmed Guests:&nbsp;<strong>{{ (int)$event->count_totalguests }}</strong></small> 
		</div>
